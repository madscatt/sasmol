...
class Topology(object):
	...
	def read_charmm_topology(self,topology_file_path='', topology_file_name='top_all27_prot_na.inp'):
		...
		error = []
		self.topology_info = {}
		lines = open(os.path.join(topology_file_path,topology_file_name),'r').readlines()
		for line in lines:
			line = line.strip()
			if (len(line)>0 and line[0]!='!'):
				...
				#
					...
					#
					if words[0]=='ATOM':
						self.add(self.topology_info[cur_res], 'ATOM', words[1:4])
					#
						...
						while (ind+2<=len(words) and words[ind][0]!='!'):
							...
							and (words[ind+1][0].isalnum() or words[ind+1][0] in ['+','-'])):
								...
								return error
							self.add(self.topology_info[cur_res], 'BOND', words[ind:ind+2])
							ind += 2
					#
						...
						while (ind+2<=len(words) and words[ind][0]!='!'):
							...
							and (words[ind+1][0].isalnum() or words[ind+1][0] in ['+','-'])):
								...
								return error
							self.add(self.topology_info[cur_res], 'DOUB', words[ind:ind+2])
							ind += 2
					#
						...
						while (ind+4<=len(words) and words[ind][0]!='!'):
							...
							and (words[ind+3][0].isalnum() or words[ind+3][0] in ['+','-'])):
								...
								return error
							self.add(self.topology_info[cur_res], 'IMPR', words[ind:ind+4])
							ind += 4
					#
						...
						while (ind+4<=len(words) and words[ind][0]!='!'):
							...
							and (words[ind+3][0].isalnum() or words[ind+3][0] in ['+','-'])):
								...
								return error
							self.add(self.topology_info[cur_res], 'CMAP', words[ind:ind+4])
							ind += 4
					#
						...
						while (ind+3<=len(words) and words[ind][0]!='!'):
							...
							and (words[ind+2][0].isalnum() or words[ind+2][0] in ['+','-'])):
								...
								return error
							self.add(self.topology_info[cur_res], 'ANGL', words[ind:ind+3])
							ind += 3
					#
						...
						while (ind+3<=len(words) and words[ind][0]!='!'):
							...
							and (words[ind+2][0].isalnum() or words[ind+2][0] in ['+','-'])):
								...
								return error
							self.add(self.topology_info[cur_res], 'THET', words[ind:ind+3])
							ind += 3
					#
						...
						while (ind+4<=len(words) and words[ind][0]!='!'):
							...
							and (words[ind+3][0].isalnum() or words[ind+3][0] in ['+','-'])):
								...
								return error
							self.add(self.topology_info[cur_res], 'DIHE', words[ind:ind+4])
							ind += 4
					#
						...
						while (ind+1<=len(words) and words[ind][0]!='!'):
							if not ( (words[ind][0].isalnum() or words[ind][0] in ['+','-'])):
								...
								return error
							self.add(self.topology_info[cur_res], 'DONO', words[ind])
							ind += 1
					#
						...
						while (ind+1<=len(words) and words[ind][0]!='!'):
							if not ( (words[ind][0].isalnum() or words[ind][0] in ['+','-'])):
								...
								return error
							self.add(self.topology_info[cur_res], 'ACCE', words[ind])
							ind += 1
					#
					elif words[0][0:4]=='IC':
						self.add(self.topology_info[cur_res], 'IC', words[1:10])
					#
					elif words[0][0:4]=='BUIL':
						self.add(self.topology_info[cur_res], 'BUIL', words[1:10])
					#
					elif words[0][0:4]=='PATC': # Need further parsing
						self.add(self.topology_info[cur_res], 'PATC', words[1:])
					#
						...
						prop = words[1][0:4]
						if prop == 'ATOM':
							self.add(self.topology_info[cur_res]['DELE'],'ATOM', words[2]) # Only 1 word  is parsed after 'DELE ATOM', which is ok for "top_all27_prot_na.inp"
						elif prop == 'ANGL':
							self.add(self.topology_info[cur_res]['DELE'],'ANGL', words[2:]) # Need further parsing
						else:
							#print 'WARNING!\n'+line+' NOT PARSED!\n' # Current "top_all27_prot_na.inp" file only has DELE ATOM and DELE ANGL, and therefore other situations are not parsed but warned
							pass
		return error
	...
	def patch_charmm_residue_atoms(self, residue, patch):
		...
		if residue not in self.topology_info:
			...
			error.append('Patch '+residue+' not found in the topology information list during patching!')
			return error
		#
		...
		self.charmm_residue_atoms[residue+'_'+patch] = numpy.array(tmp_dict['ATOM'])[:,0].tolist()
	...
	def check_charmm_atomic_order_reorganize(self):
		...
		for child_segname in children_segname:
			...
			for child in children:
				...
				#print self.compare_list_ignore_order(self.charmm_residue_atoms[child_resname], child_names)
				...
				for i in range(len(child_indices)):
						...
						self.atom()[child_indices[i]-1] = child.atom()[new_indices[i]]
						#self.index()[indices[i]-1] = child.index[new_indices[i]]
						self.name()[child_indices[i]-1] = child.name()[new_indices[i]]
						self.loc()[child_indices[i]-1] = child.loc()[new_indices[i]]
						self.resname()[child_indices[i]-1] = child.resname()[new_indices[i]]
						self.chain()[child_indices[i]-1] = child.chain()[new_indices[i]]
						self.resid()[child_indices[i]-1] = child.resid()[new_indices[i]]
						self.rescode()[child_indices[i]-1] = child.rescode()[new_indices[i]]
						self.occupancy()[child_indices[i]-1] = child.occupancy()[new_indices[i]]
						self.beta()[child_indices[i]-1] = child.beta()[new_indices[i]]
						self.segname()[child_indices[i]-1] = child.segname()[new_indices[i]]
						self.element()[child_indices[i]-1] = child.element()[new_indices[i]]
						self.charge()[child_indices[i]-1] = child.charge()[new_indices[i]]
						self.coor()[0][child_indices[i]-1] = child.coor()[0][new_indices[i]] #only frame-0 was handled in sassubset.init_child
		return error
...
def set_pdb_values(m,natoms,**kwargs):
        ...
        m.setAtom(atom) ; m.setIndex(index) ; m.setName(name) ; m.setLoc(loc) ; m.setResname(resname)
        m.setChain(chain) ; m.setResid(resid) ; m.setRescode(rescode) ; m.setOccupancy(occupancy)
        m.setBeta(beta) ; m.setSegname(segname) ; m.setElement(element) ; m.setCharge(charge)

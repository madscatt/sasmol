...
class Files(object):
	...
	def read_single_dcd_step(self,filename,frame):
		...
		#result=dcdio.read_dcdstep(infile,tx,ty,tz,num_fixed,frame,reverseEndian,charmm)
	...
	def write_pdb(self,filename,frame,flag,**kwargs):
		...
		for i in range(len(self._atom)):
			...
			try:
				...
				#infile.write("%-6s%5s %-4s%1s%-4s%1s%4s%1s   %8.3f%8.3f%8.3f%6s%6s      %-4s%2s%2s\n" % (self._atom[i],this_index,self._name[i],self._loc[i],self._resname[i],self._chain[i],self._resid[i],self._rescode[i],self._coor[frame,i,0],self._coor[frame,i,1],self._coor[frame,i,2],self._occupancy[i],self._beta[i],self._segname[i],self._element[i],self._charge[i]))
			except:
				print '\n>>>> ERROR IN WRITE_PDB <<<<\n'
				print '>> i = ',i
...
#					this_moltype = 'nucleic'
		...
		#self.initialize_children()
 		#end_time=time.time()
		#if(printme): print 'elapsed time = ',end_time - start_time

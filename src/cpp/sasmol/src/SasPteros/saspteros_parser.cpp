...
#include <sasmol.h>
#include <SasPteros/saspteros_parser.h>
...
///////////////////////////////////////////////////////////////////////////////
//  Functions for creating actual saspteros from AST
///////////////////////////////////////////////////////////////////////////////
using namespace std;
...
SasPteros_parser::SasPteros_parser(){
    has_coord = false;
}
...
void SasPteros_parser::end_token(const string& s, int& b, int i){ // i is last included index
...
void SasPteros_parser::tokenize(const string& s){
    tokens.clear();
...
struct Grammar {
    ...
    Grammar(SasPteros_parser* parent){
...
void SasPteros_parser::create_ast(string& sel_str){
#ifdef _DEBUG_PARSER
    ...
    Grammar gr(this);
    int pos = gr.run(tree);
    if(pos!=tokens.size()) throw SasPteros_error("Error near token #"+boost::lexical_cast<string>(pos));
...
void SasPteros_parser::do_optimization(AstNode_ptr& node){
...
void SasPteros_parser::apply(sasmol::SasMol* sasmol, long fr, vector<int>& result){
#ifdef _DEBUG_PARSER
	cout << "Applying to the sasmol with first atom " << sasmol->atoms[0].name << endl;
#endif
    ...
    // For coordinate-dependent saspteross perform optimization
...
void SasPteros_parser::eval_node(AstNode_ptr& node, vector<int>& result, vector<int>* subspace){
    int i,at,j,k,n;
    ...
    // Coord-independent saspteross are evaluated only once thus
        ...
        for(i=0;i<Nchildren;++i){
            ...
            if(boost::get<AstNode_ptr>(node->children[i])->code == TOK_STR){
                ...
                for(at=0;at<Natoms;++at)
                    if(mol->_atom_name()[at] == str) result.push_back(at);
            } else if(boost::get<AstNode_ptr>(node->children[i])->code == TOK_REGEX){
                // For regex
                boost::cmatch what;
                boost::regex reg(str);
                for(at=0;at<Natoms;++at){
                    if(boost::regex_match(mol->_atom_name()[at].c_str(),what,reg)){
                         result.push_back(at);
                    }
                }
            }
    ...
    case  TOK_RESNAME: {
        ...
        for(i=0;i<Nchildren;++i){
            ...
            if(boost::get<AstNode_ptr>(node->children[i])->code == TOK_STR){
                // For normal strings
                for(at=0;at<Natoms;++at)
                    if(mol->_atom_resname()[at] == str) result.push_back(at);
            } else if(boost::get<AstNode_ptr>(node->children[i])->code == TOK_REGEX){
                // For regex
                boost::cmatch what;
                boost::regex reg(str);
                for(at=0;at<Natoms;++at){
                    if(boost::regex_match(mol->_atom_resname()[at].c_str(),what,reg)){
                         result.push_back(at);
                    }
                }
            }
    ...
    case  TOK_TAG: {
        ...
        for(i=0;i<Nchildren;++i){
            ...
            if(boost::get<AstNode_ptr>(node->children[i])->code == TOK_STR){
                // For normal strings
                for(at=0;at<Natoms;++at)
                    if(mol->_atom_segname()[at] == str) result.push_back(at);
            } else if(boost::get<AstNode_ptr>(node->children[i])->code == TOK_REGEX){
                // For regex
                boost::cmatch what;
                boost::regex reg(str);
                for(at=0;at<Natoms;++at){
                    if(boost::regex_match(mol->_atom_segname()[at].c_str(),what,reg)){
                         result.push_back(at);
                    }
                }
            }
    ...
    case  TOK_CHAIN: {
        ...
        for(i=0;i<Nchildren;++i){
            ...
            for(at=0;at<Natoms;++at)
                //if(mol->_atom_chain()[at] == ch) result.push_back(at);
				// changed by ZHL
                if(mol->_atom_chain()[at][0] == ch) result.push_back(at);
        }
        return;
    }
    ...
    case  TOK_RESID: {
        ...
        // Cycle over children
        for(i=0;i<Nchildren;++i){
            // Try to get integer. If successful, go to add atoms to saspteros
            try {
                k = node->get_int_child_value(i);
                for(at=0;at<Natoms;++at)
                    // Even if k is out of range, nothing will crash here
                    if(mol->_atom_resid()[at] == k) result.push_back(at);
            } catch(boost::bad_get) {
    ...
    case  TOK_RESINDEX: {
        ...
        for(i=0;i<Nchildren;++i){
            ...
            try {
                ...
                for(at=0;at<Natoms;++at)
                    // Even if k is out of range, nothing will crash here
                    if(mol->_atoms()[at].resindex == k) result.push_back(at);
            } catch(boost::bad_get) {
                // Exception thrown, which means that this is a range, not an integer
                AstNode_ptr range = boost::get<AstNode_ptr>(node->children[i]);
                int i1 = range->get_int_child_value(0);
	...
	*/
    ...
    case  TOK_WITHIN: {
        ...
        // Do searching        
        SasPteros dum1(*mol),dum2(*mol);

        if(!subspace){
            dum1.index.resize(mol->_natoms());
            for(int i=0;i<mol->_natoms();++i) dum1.index[i] = i;
        } else {
    ...
    case  TOK_BY: {        
        ...
        for(i=0;i<Nsel;++i){ //over found atoms
            resind.insert(mol->_atoms()[res1[i]].resindex);
        }

        // Now cycle over all atoms in the sasmol (not a subset!)
        for(at=0;at<Natoms;++at){ // over all atoms
            if(resind.count(mol->_atoms()[at].resindex)){
...
float SasPteros_parser::eval_numeric(AstNode_ptr& node, int at){
    if(node->code == TOK_INT){
        ...
        return mol->_x()(at,frame);
    } else if(node->code == TOK_Y){
        return mol->_y()(at,frame);
    } else if(node->code == TOK_Z){
        return mol->_z()(at,frame);
    } else if(node->code == TOK_BETA){
        return atof(mol->_atom_beta()[at].c_str());
    } else if(node->code == TOK_OCC){
        return atof(mol->_atom_occupancy()[at].c_str());
        ...
        if(v==0.0) throw SasPteros_error("Divition by zero in saspteros!");
	...
	/*
        ...
        // Return distance
        return mol->distance(p, mol->traj[frame].coord[at] ,frame,pbc);
        ...
        Eigen::Vector3f atom = mol->traj[frame].coord[at];
        ...
        return mol->distance(atom, v, frame, pbc);
	*/
    }
}
